...
#
# FunciVar_Submit_sig05_CNVs_HGSOC_GWAS_TWAS_Loci_NewQC.R
#
...
GWAS = read.delim(paste0(New_Folder, "/", "GWAS_Loci_HRC_LD_unique_serous_hg_extra.txt"), header = T, stringsAsFactors = F, col.names = c("chrom", "start", "end"))
...
# row, chr17  41743558  43694719, contains all BRCA1 CNVs
...
fore <- read.delim("ocac_hgsoc_sig05_CNVs_hg38_unique.txt", header = T, stringsAsFactors = F)
...
back <- read.delim("ocac_hgsoc_sig05_CNVs_hg38_sim1000_new.txt", header = T, stringsAsFactors = F, na.strings = "NA")
...
GWAS_loci_LD <- read.table(file = paste0(New_Folder, "/GWAS_Loci_HRC_coord_serous_hg_extra.txt"), header = T)
head(GWAS_loci_LD)
...
GWAS_coords <- read.table(file = paste0(New_Folder, "/GWAS_Loci_HRC_coord_serous_hg_extra.txt"), header = T, sep = "\t")
...
head(GWAS_hits_bed)
write.table(x = GWAS_hits_bed, file = "HRC_GWAS_hits_hg38_sig05_NewQC_hgsoc.bed", sep = "\t", col.names = F, row.names = F, quote = F)

# must manually readd track title:
...
write.table(x = all_cnvs_ld_gwasSNP, file = "CNVs_at_GWAS_Loci_with_LD_SNP_coords_sig05_NewQC_hgsoc.txt", quote = F, sep = "\t", row.names = F, col.names = T)
...
write.table(x = only_ld_gwasSNP, file = "CNVs_at_GWAS_Loci_only_LD_SNP_coords_sig05_NewQC_hgsoc.txt", quote = F, sep = "\t", row.names = F, col.names = T)
...
write.table(x = gwas_overlap_cnvs, file = "CNVs_at_GWAS_Loci_sig05_NewQC_hgsoc.txt", quote = F, sep = "\t", row.names = F, col.names = T)
...
write.table(x = enrichment, file = "GWAS_TWAS_enrichment_results_sig05_NewQC_hgsoc.txt", quote = F, sep = "\t", row.names = F, col.names = T)


enrichment_short <- dplyr::select(enrichment, sample, fg.ratio, bg.ratio, probability, difference, fg.success, fg.total, bg.success, bg.total, significant)

write.table(x = enrichment_short, file = "GWAS_TWAS_enrichment_results_short_sig05_NewQC_hgsoc.txt", quote = F, sep = "\t", row.names = F, col.names = T)
