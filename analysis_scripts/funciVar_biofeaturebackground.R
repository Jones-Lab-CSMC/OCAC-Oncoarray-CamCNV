#funciVar_biofeaturebackground.R

###
### Code edited from Simon Coetzee's FunciVar on github
### Original code pulled from https://github.com/Simon-Coetzee/funcivar/blob/master/R/funcivar.R on 20190606
### Bubble plot code edited from code provided by Alberto Reyes
### Edited by Amber DeVries
###
...
#'   performed against them. Must have metadata columns named "sample" and "state", 
#'   "feature" and "signalValue", or simply "type".
...
#'   foreground is a strict subset of the background, or an independent set.
...
CalculateEnrichment <- function(variants, features, feature.type = "biofeatures",
  ...
  if (strict.subset == "guess") {
    ...
    } else if (all(make.unique(as.character(variants$fg)) %in% make.unique(as.character(variants$bg)))) {
      #originally: else if (all(rownames(variants$fg) %in% rownames(variants$bg))) which always returned true if the rownames were NULL
      strict.subset <- TRUE
      # see the following:
    } else {
      strict.subset <- FALSE
  ...
  if (!missing(features)) {
    if (feature.type == "biofeatures") {
      ...
      print("finished overlapping foreground")
      variants$bg <- SetOverlaps(variants$bg, features)
      print("finished overlapping background")
      #which features are in the overlaps matrix
      fg.features <- colnames(mcols(ShowOverlaps(variants$fg)))
      bg.features <- colnames(mcols(ShowOverlaps(variants$bg)))
      #features overlapped in both foreground and background
      ...
      if (any(!(is.element(all.features, fg.features)))) {
        if (is(variants$fg, "VCF")) {
          ...
          # if any feature is in bg but not in fg, set all row values for that column to 0L in fg
      ...
      if (is(variants$bg, "VCF")) {
        ...
        # add the original metadata columns and the new overlap feature columns to the metadata
      ...
      if (any(!(is.element(all.features, bg.features)))) {
        if (is(variants$bg, "VCF")) {
          ...
          # if any feature is in fg but not in bg, set all row values for that column to 0L in bg
    ...
    #####################################################################################################
    if (feature.type == "variants") {
      if(inherits(variants, "GRangesList")) variants <- unlist(variants, use.names = FALSE)
      overlap_list <- list(fg=SetOverlaps(features, variants$fg, feature.variants = TRUE), 
                           bg=SetOverlaps(features, variants$bg, feature.variants = TRUE))
      #which features are in the overlaps matrix
      fg.features <- colnames(mcols(ShowOverlaps(overlap_list$fg)))
      bg.features <- colnames(mcols(ShowOverlaps(overlap_list$bg)))
      #features overlapped in both foreground and background
      all.features <- union(fg.features, bg.features)
      ## first fg
      if (any(!(is.element(all.features, fg.features)))) {
        if (is(overlap_list$fg, "VCF")) {
          mcols(rowRanges(overlap_list$fg))[, all.features[!(is.element(all.features, fg.features))]] <- 0L
        } else if (is(overlap_list$fg, "GRanges")) {
          # if any feature is in bg but not in fg, set all row values for that column to 0L in fg
          mcols(overlap_list$fg)[, all.features[!(is.element(all.features, fg.features))]] <- 0L
        }
      }
      if (is(overlap_list$bg, "VCF")) {
        mcols(rowRanges(overlap_list$fg)) <- cbind(mcols(rowRanges(overlap_list$fg))
                                                   [, 1:metadata(overlap_list$fg)$overlap.offset-1], 
                                                   mcols(rowRanges(overlap_list$fg))[, all.features, drop = FALSE])
      } else if (is(overlap_list$fg, "GRanges")) {
        # add the original metadata columns and the new overlap feature columns to the metadata
        mcols(overlap_list$fg) <- cbind(mcols(overlap_list$fg)[, 1:attributes(overlap_list$fg)$metadata$overlap.offset-1], 
                                        mcols(overlap_list$fg)[, all.features, drop = FALSE])
      }
      ## then bg
      if (any(!(is.element(all.features, bg.features)))) {
        if (is(overlap_list$bg, "VCF")) {
          mcols(rowRanges(overlap_list$bg))[, all.features[!(is.element(all.features, bg.features))]] <- 0L
        } else if (is(overlap_list$bg, "GRanges")) {
          # if any feature is in fg but not in bg, set all row values for that column to 0L in bg
          mcols(overlap_list$bg)[, all.features[!(is.element(all.features, bg.features))]] <- 0L
        }
      }
      if (is(overlap_list$bg, "VCF")) {
        mcols(rowRanges(overlap_list$bg)) <- cbind(mcols(rowRanges(overlap_list$bg))
                                                   [, 1:metadata(overlap_list$bg)$overlap.offset-1], 
                                                   mcols(rowRanges(overlap_list$bg))[, all.features, drop = FALSE])
      } else if (is(overlap_list$bg, "GRanges")) {
        # add the original metadata columns and the new overlap feature columns to the metadata
        mcols(overlap_list$bg) <- cbind(mcols(overlap_list$bg)
                                        [, 1:attributes(overlap_list$bg)$metadata$overlap.offset-1], 
                                        mcols(overlap_list$bg)[, all.features, drop = FALSE])
      }
    }
  }
  if (feature.type == "biofeatures") {
  ...
  #############################################################################
  } else if (feature.type == "variants") {
    enrichment <- enrich.features(fg = overlap_list$fg, bg = overlap_list$bg, CI = CI, prior = prior, strict.subset = strict.subset)
    if(return.overlaps) {
      if(all(names(mcols(overlap_list$fg)) == names(mcols(overlap_list$bg)))) {
        list.fun <- GenomicRanges::GRangesList
      } else {
        list.fun <- S4Vectors::List
      }
      if (is(overlap_list$fg, "GRanges") & is(overlap_list$bg, "GRanges")) {
        overlaps <- list.fun(foreground.overlaps = overlap_list$fg, background.overlaps = overlap_list$bg)
      } else {
        overlaps <- list.fun(foreground.overlaps = rowRanges(overlap_list$fg), background.overlaps = rowRanges(overlap_list$bg))
      }
      return(list(overlaps = overlaps, enrichment = enrichment))
    } else {
      return(enrichment)
    ...
    stop("feature.type: ", feature.type, "is not availible; try 'biofeatures', 'segmentations', or 'variants'")
  }
  return(enrichment)
}
...
#'   annotate variants. Must have metadata columns named "sample" and "state", 
#'   "feature" and "signalValue", or simply "type".
#'
...
#' @export
SetOverlaps <- function(variants, features, feature.variants = F) {
  if (feature.variants == FALSE) {
    ...
    if(all(names(mcols(features)) == c("sample", "state"))) {
      ...
      id.name <- "feature"
    } else if (all(names(mcols(features)) == "type")) {
      mcols(nfeatures)$type <- "none"
      id.name <- "type"
    } # added this so it would run enrichment by a single column such as "sample.state" 
    # rather than only on "sample" or "feature"
    ...
    overlapsdf <- data.frame(from = names(variants)[from(overlaps)],
                           to = mcols(features)[to(overlaps), id.name],
                           stringsAsFactors = FALSE)
    resmatrix <- make.overlap.matrix(overlapsdf)
    #remove the column with "none" (either a "sample" or a "feature" column) (also only keep overlaps in "variants")
    resmatrix <- resmatrix[names(variants), !grepl("^none$", colnames(resmatrix)), drop = FALSE]
    if(ncol(resmatrix) > 0L) {
      # if there are any overlaps in the resmatrix
      if(is(variants, "GRanges")) {
        overlap.offset <- ncol(mcols(variants)) + 1
        # add resmatrix data to elementMetadata columns (for foreground or background)
      ...
      if(is(variants, "GRanges")) {
        print(paste("ncol(mcols(features)) is", ncol(mcols(features))))
        print(paste("ncol(mcols(variants)) is", ncol(mcols(variants))))
    ...
    #variants version
    if(all(names(mcols(features)) == c("sample", "state"))) {
      names(mcols(features)) <- c("type", "state")
      #print("sample column renamed to 'type' and state column dropped")
      id.name <- "sample"
    } else if (all(names(mcols(features)) == c("feature", "signalValue"))) {
      names(mcols(features)) <- c("type", "signalValue")
      id.name <- "feature"
    } else if (all(names(mcols(features)) == "type")) {
      id.name <- "type"
    } # added this so it would run enrichment by a single column such as "sample.state" 
    ...
    if (is.null(names(features))) {
      names(features) <- make.unique(as.character(features))
    }
    
    alltypes <- unique(mcols(features)$type)
    
    alloverlaps <- data.frame(from = character(), to = character(), stringsAsFactors = FALSE)
    
    for (typeid in alltypes) {
      subfeat <- subset(features, type == typeid)
      overlaps <- findOverlaps(variants, subfeat, ignore.strand = TRUE)
      overlaps <- data.frame(from = names(subfeat)[to(overlaps)],
                             to = mcols(subfeat)[to(overlaps), "type"],
                             stringsAsFactors = FALSE)
      alloverlaps <- rbind(alloverlaps, overlaps)
      
    }
    
    resmatrix <- make.overlap.matrix(alloverlaps)
    #remove the column with "none" (either a "sample" or a "feature" column) (also only keep overlaps in "features")
    #select all rownames 
    # now edit the resmatrix to be full matrix of length of features
    fullmatrix <- matrix(data = rep(0, length(names(features))*ncol(resmatrix)), nrow = length(names(features)), 
                         ncol = ncol(resmatrix), 
                         dimnames = list(names(features), colnames(resmatrix)))
    #initialize a matrix of zeros with correct row and column names
    
    fullrows <- match(dimnames(resmatrix)[[1]], dimnames(fullmatrix)[[1]])
    resrows <- match(dimnames(fullmatrix)[[1]], dimnames(resmatrix)[[1]])
    resrows <- resrows[!is.na(resrows)]
    
    # this is to ensure the column names are the same in the rows (if I changed the fullmatrix initialization)
    bothcols <- intersect(dimnames(resmatrix)[[2]], dimnames(fullmatrix)[[2]])
    
    fullmatrix[fullrows,bothcols] <- resmatrix[resrows,bothcols]
    
    
    #use fullmatrix below
    if(id.name == "sample") {
      names(mcols(features)) <- c("sample", "state")
    } else if (id.name == "feature") {
      names(mcols(features)) <- c("feature", "signalValue")
    }
    
    if(ncol(fullmatrix) > 0L) {
      # if there are any overlaps in the fullmatrix
      if(is(features, "GRanges")) {
        overlap.offset <- ncol(mcols(features)) + 1
        # add fullmatrix data to elementMetadata columns (for foreground or background)
        mcols(features) <- cbind(mcols(features), DataFrame(fullmatrix))
        attributes(features)$metadata$overlap.offset <- overlap.offset
      } else if (is(features, "VCF")) {
        overlap.offset <- (ncol(mcols(rowRanges(features))) + 1) - 4
        mcols(rowRanges(features)) <- cbind(mcols(rowRanges(features)), DataFrame(fullmatrix))
        metadata(features)$overlap.offset <- overlap.offset
      } else {
        stop("features must be either a GRanges or VCF object, your object is: ", class(features))
      }
    } else {
      if(is(features, "GRanges")) {
        metadata(features)$overlap.offset <- ncol(mcols(features)) + 1
      } else if (is(features, "VCF")) {
        metadata(features)$overlap.offset <- ncol(mcols(rowRanges(features))) + 1
      } else {
        stop("features must be either a GRanges or VCF object, your object is: ", class(features))
      }
      warning("No overlaps were found for this variant/feature combination")
...
ShowOverlaps <- function(variants, feature = NULL) {
  ...
  if (ncol(mcols(variants)) < offset) {
    print("returning blank dataframe")
    return(DataFrame())
  }
  if (is.null(feature)) {
    if (!is.null(offset)) {
...
#'   CalculateEnrichment()
#'
#' @param graphtitle one of 'difference' or 'log.odds.ratio', thereby plotting the
#'   difference in beta distributions between foreground and background, or the
#'   oddsratio of the comparison.
#' @param pdfname the filename you would like 
#' @param filepath optional file path such as "Folder1/Folder2/" to output the 
#'   saved figure pdf to
#'
...
#' @export
PlotBubblePlot <- function(variant.enrichment = NULL, 
                                  graphtitle = "FunciVar Bubble Plot of Enrichment", 
                                  pdfname = "FunciVar_Bubble_Plot_of_Enrichment.pdf", 
                                  filepath = "") {
  
  if(is.null(variant.enrichment$state)) {
    stop("no state column for variant.enrichment")
  }
  
  # need to add state and celltype to variant.enrichment beforehand
  if(is.null(variant.enrichment$celltype)) {
    stop("no celltype column for variant.enrichment")
  }
  
  
  bubble_plot <- ggplot(variant.enrichment, aes(y = sample, x = state)) +
    ...
    theme_bw() +
    scale_color_distiller(direction = -1, palette = "PRGn", limits = c(-1,1)*max(variant.enrichment$difference)) +
...
#' @importFrom stringr str_length
#' @export
PlotBubblePlot.horizontal <- function(variant.enrichment = NULL, 
                           graphtitle = "FunciVar Bubble Plot of Enrichment", 
                           pdfname = NULL, 
                           jpgname = NULL,
                           filepath = getwd(), plot_height=6, plot_width=11) {
  
  #figure out how to auto add state in the CalculateEnrichment code
  if(is.null(variant.enrichment$state)) {
    stop("no state column for variant.enrichment")
  }
  ...
  if(is.null(variant.enrichment$celltype)) {
    ...
    bubble_plot <- ggplot(data = variant.enrichment, aes(x = sample, y = state)) +
      geom_point(aes(size = probability, color = difference)) +
      geom_point(data = variant.enrichment[variant.enrichment$significant == T,], color = "black", aes(shape = "a", stroke = 2, size = probability)) +
      scale_shape_discrete(solid=F, name = "significant", labels = "true") +
      theme_bw() +
      scale_color_distiller(direction = -1, palette = "PRGn", limits = c(-1,1)*max(abs(variant.enrichment$difference))) +
      theme(legend.position="right",
            panel.grid.major = element_blank(),
            ...
            strip.text.x = element_text(angle = 90, hjust = 0, vjust = 0.5, size = rel(1.5)),
            strip.text.y = element_text(angle = 0, hjust = 0.5, vjust = 1, size = rel(1.5)),
            strip.background = element_blank(),
            panel.spacing = unit(.1, "lines")) +
      ggtitle(graphtitle) #+
...
enrich.features <- function(fg, bg, CI, prior, strict.subset) {
  ## foreground overlaps
  print(ShowOverlaps(fg))
  print(mcols(ShowOverlaps(fg)))
  ...
  enrichment <- data.frame(sample = character(),
                           ...
                           bg.ratio = numeric(),
                           probability = numeric(),
                           #these three log.odds.* columns were added to enrichment
                           log.odds.ratio = numeric(),
                           log.odds.lower = numeric(),
                           log.odds.upper = numeric(),
                           difference = numeric(),
                           lower = numeric(),
                           upper = numeric(),
                           fg.success = numeric(),
  ...
  for (my.sample in seq_along(colnames(bg.over.matrix))) {
    ...
    n1 <- sample.size
    y1 <- sample.success
    n2 <- total.size - sample.size # this is only subtracted for strict.subset
    ...
    result <- data.frame(sample = colnames(bg.over.matrix)[my.sample],
                         ...
                         probability = abs(mean(theta1 > theta2) - mean(theta1 < theta2)),
  ...
  enrichment[enrichment$probability > (1 - CI), "significant"] <- TRUE
  return(enrichment)
}
...
#' #'   CalculateEnrichment()
#' #'
#' #' @param value one of 'difference' or 'log.odds.ratio', thereby plotting the
#' #'   difference in beta distributions between foreground and background, or the
#' #'   oddsratio of the comparison.
