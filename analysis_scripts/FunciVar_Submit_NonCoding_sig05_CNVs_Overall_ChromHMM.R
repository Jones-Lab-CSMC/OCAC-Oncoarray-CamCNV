...
#
# FunciVar_Submit_NonCoding_sig05_CNVs_Overall_ChromHMM.R
#
...
if(!is.na(opt$mark)) {
  ...
  fore <- read.delim(paste0(New_Folder, "/", "ocac_overall_sig05_noncoding_CNVs_hg38.txt"), header = T, stringsAsFactors = F)
  ...
  back <- read.delim(paste0(New_Folder, "/", "ocac_overall_sig05_noncoding_CNVs_hg38_sim2000.txt"), header = T, stringsAsFactors = F, na.strings = "NA")
  ...
  save(enrichment, file = paste0(New_Folder, "/", opt$mark, "_ChromHMM_overall_sig05_noncoding_enrichment.RData"))
